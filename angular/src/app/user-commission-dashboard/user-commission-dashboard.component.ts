...
@Component({
  selector: 'app-user-commission-dashboard',
  templateUrl: './user-commission-dashboard.component.html',
  styleUrls: ['./user-commission-dashboard.component.css']
})
export class UserCommissionDashboardComponent implements OnInit {
  ...
  faCheck = faCheck;
    ...
    displayedColumns: string[] = ['id', 'name', 'atsign','orderAmount', 'finalCommission'];
    ...
    ngOnInit() {
        ...
        this.getCommercialAtsignByUser(data);
    }
    // ngAfterViewInit() {
    ...
    getCommercialAtsignByUser(data) {
        ...
        this.userService.getCommercialAtsignByUser(data).subscribe(
            res => {
                this.allAtsigns = res['data'] && res['data']['records'] ? res['data']['records'] : [];
                console.log(this.allAtsigns);
